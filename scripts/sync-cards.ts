...
async function main() {
  ...
  if (args.includes("--languages")) {
    // English + Japanese — the two languages bindr surfaces.
    console.log("=== Full EN sync ===");
    const enSets = await listSetIds("en");
    let en = 0;
    for (const s of enSets) {
      const n = await syncSet("en", s);
      en += n;
      if (n) console.log(`  ✓ en:${s} — ${n}`);
    }
    console.log(`EN: ${en} cards across ${enSets.length} sets`);
